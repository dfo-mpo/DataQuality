...
""" Class to manage and run all metrics for the Relevance dimension. 
...
"""
class Relevance:
    # Finds and loads all metric names defined in the relevance folder
    METRIC_FOLDER = os.path.dirname(__file__) 
    ALL_METRICS = core_operations.list_metric_names(METRIC_FOLDER)

    """ Collects metadata class from all metrics in the relevance folder.
    """
    @classmethod
    ...
    def __init__(self, dataset_path, return_type="score", logging_path=None, uploaded_file_name=None, metric_params=None):
        ...
        # Looks for all metric classes in the relevance folder
        self.ALL_METRICS = self.search_metrics() 

    """ Automatically finds and loads all metric classes defined in the relevance folder.
    """
    ...
    """ Run metrics: Will run specified metrics or all relevance metrics by default. return_logs returns the logging data so the UI can visualize test output details.
    """
        ...
        if set(metrics).issubset(set(self.ALL_METRICS)):
            ...
            for metric in metrics:
                ...
                try:
                    ...
                    relevance_score, metric_log_csv = metric_instance.run_metric()
