...
""" Class to manage and run all metrics for the Completeness dimension. 
...
"""
class Completeness:
    # Finds and loads all metric names defined in the completeness folder
    METRIC_FOLDER = os.path.dirname(__file__) 
    ALL_METRICS = core_operations.list_metric_names(METRIC_FOLDER)

    """ Collects metadata class from all metrics in the completeness folder.
    """
    @classmethod
    ...
    def __init__(self, dataset_path, return_type="score", logging_path=None, uploaded_file_name=None, metric_params=None):
        ...
        # Looks for all metric classes in the completeness folder
        self.ALL_METRICS = self.search_metrics()

    """ Automatically finds and loads all metric classes defined in the completeness folder.
    """
    ...
    """ Run metrics: Will run specified metrics or all completeness metrics by default. return_logs returns the logging data so the UI can visualize test output details.
    """
        ...
        if set(metrics).issubset(set(self.ALL_METRICS)):
            ...
            for metric in metrics:
                ...
                try:
                    ...
                    completeness_score, metric_log_csv = metric_instance.run_metric()
