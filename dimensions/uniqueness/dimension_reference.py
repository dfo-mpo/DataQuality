...
""" Class to manage and run all tests for the Uniqueness dimension. 
...
"""
class Uniqueness:
    # Finds and loads all test names defined in the uniqueness folder
    DIM_FOLDER = os.path.dirname(__file__) 
    ALL_TESTS = core_operations.list_test_names(DIM_FOLDER)

    """ Collects metadata class from all tests in the uniqueness folder.
    ...
    def __init__(self, dataset_path, return_type="score", logging_path=None, uploaded_file_name=None, test_params=None):
        ...
        # Looks for all test classes in the uniqueness folder
        self.ALL_TESTS = self.search_tests() 

    """ Automatically finds and loads all test classes defined in the uniqueness folder.
    """
    ...
    """ Runs specified tests or all uniqueness tests by default. 
    ...
    def run_tests(self, tests=None, return_logs=False):
        ...
        if set(tests).issubset(set(self.ALL_TESTS)):
            ...
            for test in tests:
                ...
                overall_uniqueness_score = {"test": None, "value": None}  # Ensure it exists even if errors occur
                try:
                    ...
                    overall_uniqueness_score["test"] = test
                    uniqueness_score, test_log_csv = test_instance.run_test()
