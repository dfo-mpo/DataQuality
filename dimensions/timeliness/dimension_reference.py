...
""" Class to manage and run all tests for the Timeliness dimension. 
...
"""
class Timeliness:
    # Finds and loads all test names defined in the timeliness folder
    DIM_FOLDER = os.path.dirname(__file__) 
    ALL_TESTS = core_operations.list_test_names(DIM_FOLDER)

    """ Collects metadata class from all tests in the timeliness folder.
    ...
    def __init__(self, dataset_path, return_type="score", logging_path=None, uploaded_file_name=None, test_params=None):
        ...
        # Looks for all test classes in the timeliness folder
        self.ALL_TESTS = self.search_tests() 

    """ Automatically finds and loads all test classes defined in the timeliness folder.
    """
    ...
    """ Runs specified tests or all timeliness tests by default. 
    ...
    def run_tests(self, tests=None, return_logs=False):
        ...
        if set(tests).issubset(set(self.ALL_TESTS)):
            ...
            for test in tests:
                ...
                overall_timeliness_score = {"test": None, "value": None}  # Ensure it exists even if errors occur
                try:
                    ...
                    overall_timeliness_score["test"] = test
                    timeliness_score, test_log_csv = test_instance.run_test()
