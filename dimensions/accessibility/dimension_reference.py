...
""" Class to manage and run all tests for the Accessibility dimension. 
...
"""
class Accessibility:
    # Finds and loads all test names defined in the accessibility folder
    DIM_FOLDER = os.path.dirname(__file__) 
    ALL_TESTS = core_operations.list_test_names(DIM_FOLDER)

    """ Collects metadata class from all tests in the accessibility folder.
    ...
    def __init__(self, dataset_path, return_type="score", logging_path=None, uploaded_file_name=None, test_params=None):
        ...
        # Looks for all test classes in the accessibility folder
        self.ALL_TESTS = self.search_tests() 

    """ Automatically finds and loads all test classes defined in the accessibility folder.
    """
    ...
    """ Runs specified tests or all accessibility tests by default. 
    ...
    def run_tests(self, tests=None, return_logs=False):
        ...
        if set(tests).issubset(set(self.ALL_TESTS)):
            ...
            for test in tests:
                ...
                overall_accessibility_score = {"test": None, "value": None}  # Ensure it exists even if errors occur
                try:
                    ...
                    overall_accessibility_score["test"] = test
                    accessibility_score, test_log_csv = test_instance.run_test()
